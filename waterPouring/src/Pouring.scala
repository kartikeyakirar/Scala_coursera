  ...
  case class Pour(from: Int, to: Int) extends Move {
   ...
   val moves =
    ...
    (for (from <- glasses; to <- glasses if from != to) yield Pour(from, to))
      ...
      /*val endState = trackhistory(history)
    private def trackhistory(his:List[Move]):State= his match{
      case Nil => initialState
      case head :: xs1=> head change trackhistory(xs1)
    }
    */   
      
  class Path(history:List[Move]){
    val endState= (history foldRight initialState )(_ change _)
    def extend(move:Move)=new Path(move::history)
    override def toString = (history.reverse mkString " ") + "--> " + endState
      
    val initialPath = new Path(Nil)
    
 /*   def form(paths:Set[Path]):Stream[Set[Path]]=

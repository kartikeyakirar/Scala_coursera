...
object kl {;import org.scalaide.worksheet.runtime.library.WorksheetSupport._; def main(args: Array[String])=$execute{;$skip(72); 
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def isEmpty= false
    ...
    def descendingByRetweet: TweetList={
      def makeretweetlist(l:TweetSet,r:TweetList):TweetList={
      if(l.isEmpty) r
      else makeretweetlist(l.remove(l.mostRetweeted),r.incl(l.mostRetweeted) )}
       
      makeretweetlist(this,Nil)
    }
    /**
   * The following methods are already implemented
   */
...
trait TweetList {
  ...
  def tail: TweetList
  def isEmpty: Boolean
  def incl(tweet:Tweet):TweetList={ new Cons(tweet,this)}
  def foreach(f: Tweet => Unit): Unit =
    if (!isEmpty) {
      f(head)
      tail.foreach(f)
